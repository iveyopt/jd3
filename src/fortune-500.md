...
function launchTimeline(data, {width} = {}) {
  return Plot.plot({
    ...
    marks: [
      Plot.rectY(data, Plot.binX({y: "count"}, {x: "founded", fill: "industry"})),
      Plot.ruleY([0])
    ]
  });
}
...
<div class="grid grid-cols-1">
  <div class="card">
    ${resize((width) => launchTimeline(data, {width}))}
  </div>
</div>
...
d3.csv("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/heatmap_data.csv", function(data) {
  ...
  var myGroups = d3.map(data, function(d){return d.industry;}).keys()
  var myVars = d3.map(data, function(d){return d.brandedSubreddit;}).keys()
